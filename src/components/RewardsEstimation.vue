...
<div class="rewardsestimation">
    <div class="rewardsestimation_row">
        <h3 id="rew_est_sl1">{{ $t('rewards_estimation.your_stake') }} <span class="rewardsestimation_highlighted">{{ formatNumberByLocale(stake, 0) }}</span></h3>
        <FSlider
            name="stake"
            v-model="stake"
            use-lower-fill-bar
    ...
    <div class="rewardsestimation_rewards">
        <div>
            ...
            <div class="rewardsestimation_highlighted">
                <template v-if="yApr > 0">{{ formatNumberByLocale(rewards, 0) }}</template>
